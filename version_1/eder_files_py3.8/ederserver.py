	...
	class EderHandler(BaseHTTPServer.BaseHTTPRequestHandler, object):
		...
		def do_GET(self):
			...
			if request == 'setref':
				...
				# LNA OFF
				self.eder_obj.rx.dco.run(trx_ctrl=0x01, rx_dco_en=0x01, trx_rx_on=0x1FFFFF, trx_rx_off=0x1F0000, gain=((None,None),None,None,None), beam=self.eder_obj.rx.get_beam(), lna=0, verbose=1)
				print 'Ran DC-offset calibration'
			...
			elif request == 'rxloadspecificbeambook':
				beambook = split_path[2]
				s = 'lut/beambook/{}'.format(beambook)
        			freq = self.get_rf_freq()
				self.eder_obj.rx.bf.idx.setup('lut/beambook/bf_rx_awv_idx_rfm_3.0_R2.0', 64.80e9)
				self.eder_obj.rx.bf.awv.setup(s, freq, 0.0)
			elif request == 'txloadspecificbeambook':
				beambook = split_path[2]
				s = 'lut/beambook/{}'.format(beambook)
        			freq = self.get_rf_freq()
				self.eder_obj.tx.bf.idx.setup('lut/beambook/bf_tx_awv_idx_rfm_3.0_R2.0', 64.80e9)
				self.eder_obj.tx.bf.awv.setup(s, freq, 0.0)
			...
			elif request == 'txawvdump':
				self.eder_obj.tx.bf.awv.dump()
			elif request == 'rxawvdump':
