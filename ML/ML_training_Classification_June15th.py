...
for fx in range(1):
    ## Load Data
    X_complex, y = np.load('./Datasets/channels_1.npy')[:],np.load('./Datasets/locs_1.npy') 
    ...
    nb_classes = np.max(y)+1 # number of unique digits    
    Y_train = np_utils.to_categorical(y_train, nb_classes) ## Get One hot vector for each beam
    ...
    #Input Layer
    model.add(Dense(512, input_shape=(size_estimates,))) 
    ...
    topk_metrics = [ keras.metrics.TopKCategoricalAccuracy(name='Top-%i'%k, k = k) for k in [2, 3, 5]]
