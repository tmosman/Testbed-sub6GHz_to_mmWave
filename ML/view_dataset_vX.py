...
for i,loc in enumerate(locs):
    all_ch_est = sub6[loc]
    all_pwr_est = np.tile(i+1,[all_ch_est.shape[0],1])
   ...
   # for 
    ...
    #all_ch_est[0,:]
np.save('./Datasets/channels_1.npy',all_ch_est[1:,:])
np.save('./Datasets/locs_1.npy',all_pwr_est[1:,:].astype(int)) 
print(np.load('./Datasets/channels_1.npy').shape)
print(np.load('./Datasets/locs_1.npy').shape)
'''
int_sub6 = np.zeros([1,256])
int_pwr = np.zeros([1,1])
for loc in X_train:
