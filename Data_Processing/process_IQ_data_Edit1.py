...
for k in range(1):
    ...
    usrp2_data = usrp1_data.reshape(-1,2).transpose()
    all_data = np.vstack((usrp1_data,usrp2_data))
    ...
    for ch_idx in range(4):
        plt.plot(all_data[ch_idx,0:100])
        plt.pause(1)
        ...
        # print(f'RF: {ch_idx}')
            # Hest,Hest_0 = estObj.Rx_processing_Hest(data.reshape(1,-1),0,min(all_pkts)-2)
            # ph_diff = np.angle(Hest[0,1:])- np.angle(Hest_0[0,1:])
            # ph_diff[26:38] = np.zeros((12,))
            # plt.title(f'Capture: {k} RF: {ch_idx} window: {q}')
            # plt.plot(ph_diff)
            # plt.pause(0.1)
            # if np.var(ph_diff,axis=0) > 0.7:
            #     print('---------------------------------- ')
            #     print(f' Capture: {k} RF: {ch_idx} window: {q}')
            #     print(np.var(ph_diff,axis=0))
            #     print('---------------------------------- ')
                #plt.pause(5)
    print(f'Capture Time : {time.time()-t1}')
# for usrp in range(2):    
#     for k in range(10):
