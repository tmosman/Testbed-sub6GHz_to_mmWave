...
for loc in range(1,21):
    ...
    for k in range(10):
        ...
        np.save(f'./Hest_data/channel_estimates_{k}.npy',np.array(u1))               
    print(f'Time capture {loc} : {(time.time()-tx)/60} minutes')
    np.save(f'./Hest_data/capture_{loc}_channel_estimates.npy',np.array(all_caps))  
